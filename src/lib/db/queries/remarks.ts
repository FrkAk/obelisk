 ...
 *
 * @returns Object mapping column aliases to Drizzle column references.
 */
 ...
 *
 * @param row - Flat row from a remarkPoiSelect query.
 * @returns Nested RemarkWithPoi with poi and category objects.
 */
 ...
 *
 * @param params - POI ID, locale, and the generated remark data.
 * @returns The inserted remark row.
 */
 ...
 * When locale is provided, prefers locale-specific remarks but falls back to any current remark.
 *
 * @param poiIds - Array of POI UUIDs.
 * @param locale - Optional locale string to filter remarks by.
 * @returns Array of current remarks with their POIs and categories.
 */
export async function getRemarksByPoiIds(
  ...
  const baseCondition = and(inArray(remarks.poiId, poiIds), eq(remarks.isCurrent, true));
 ...
 *
 * @param remarkId - The remark's UUID.
 * @returns The remark with its POI and category, or undefined.
 */
 ...
 * When locale is provided, prefers locale-specific remark but falls back to any current remark.
 *
 * @param poiId - The POI's UUID.
 * @param locale - Optional locale string to filter by.
 * @returns The current remark with its POI, or undefined.
 */
export async function getCurrentRemarkForPoi(
  poiId: string,
  locale?: string,
): Promise<RemarkWithPoi | undefined> {
  const baseCondition = and(eq(remarks.poiId, poiId), eq(remarks.isCurrent, true));
  ...
  if (locale) {
    ...
    if (localeResults[0]) return mapRowToRemarkWithPoi(localeResults[0]);
  }
 ...
 *
 * @param poiId - The POI's UUID.
 * @returns Array of all remark versions for the POI.
 */
 ...
 *
 * @param oldRemarkId - The ID of the current remark to supersede.
 * @param poiId - The POI's UUID.
 * @param remark - The full generated remark data.
 * @returns The newly inserted remark row.
 */
 ...
 *
 * @param poiId - The POI's UUID.
 * @returns The highest version number, or 0 if no remarks exist.
 */
