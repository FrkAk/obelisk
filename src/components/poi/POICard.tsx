...
export function POICard({
  ...
  const TABS = useMemo<{ id: TabId; label: string }[]>(() => [
    { id: "remark", label: t("poi.tabRemark") },
    { id: "capsules", label: t("poi.tabCapsules") },
    { id: "details", label: t("poi.tabDetails") },
  ], [t]);
    ...
    >
      ...
      />
        ...
        >
          ...
          >
            {categoryDisplay}
            {poi.distance != null && poi.distance > 0 && ` · ${formatDistance(poi.distance)} ${t("poi.away")}`}
          </span>
        </motion.div>
      </div>
...
function ShareButton({ poiId }: { poiId: string }) {
  ...
  const handleShare = async () => {
    const url = `${window.location.origin}/poi/${poiId}`;
    try {
      ...
      {!isGenerating && !isRegenerating && hasRemark && remark && (
        ...
        {onRegenerate && (
          ...
          >
            ...
            {cooldownRemaining > 0 ? `${cooldownRemaining}s` : t("remark.regenerate")}
          </motion.button>
        )}
        </motion.div>
      )}
      ...
      <div className="space-y-3">
        <div className="flex flex-wrap gap-1.5">
          {(["text", "voice", "photo", "video"] as const).map((format) => {
            const label = t(`capsules.${format}` as "capsules.text" | "capsules.voice" | "capsules.photo" | "capsules.video");
            return (
              <span
                key={format}
        ...
        <textarea
          ...
          rows={4}
          placeholder={t("capsules.sharePlaceholder")}
        />
...
function DetailsTab({ poi }: { poi: ExternalPOI }) {
  ...
  if (poi.hasWifi) amenities.push(t("details.wifiAvailable"));
  if (poi.hasOutdoorSeating) amenities.push(t("details.outdoorSeating"));
