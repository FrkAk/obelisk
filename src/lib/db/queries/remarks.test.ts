...
describe("mapRowToRemarkWithPoi", () => {
  ...
  test("locale field is mapped correctly", () => {
    const row = makeRow({ remarkLocale: "tr" });
    const result = mapRowToRemarkWithPoi(row);
    expect(result.locale).toBe("tr");
  });

  test("null locale is preserved", () => {
    const row = makeRow({ remarkLocale: null });
    const result = mapRowToRemarkWithPoi(row);
    expect(result.locale).toBeNull();
  });

  test("poi locale is mapped from row", () => {
    const row = makeRow({ poiLocale: "tr-TR" });
    const result = mapRowToRemarkWithPoi(row);
    expect(result.poi.locale).toBe("tr-TR");
  });
});
