 ...
 *
 * @param poi - POI with optional category data.
 * @returns ExternalPOI suitable for the POI card component.
 */
...
export default function Home() {
  const { locale, cityCenter, t } = useLocale();
  ...
  useEffect(() => {
    if (searchError) setAppError(t("error.searchDown"));
  }, [searchError, t]);
  ...
  const handlePoiClick = useCallback(
    async (poi: { name: string; latitude: number; longitude: number; category?: string }) => {
      ...
      try {
        ...
        if (!response.ok) {
          setAppError(t("error.cantLoadPlace"));
          setSheetOpen(false);
          setSheetMode(null);
          return;
        }
  ...
  const handleGenerateRemarkForPoi = useCallback(async () => {
    ...
    try {
      const response = await fetch("/api/remarks/generate-for-poi", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ poi: selectedPoi, locale }),
      });

      if (!response.ok) {
        if (response.status === 422) {
          setAppError(t("error.cantGenerateInsufficient"));
        } else if (response.status === 503) {
          setAppError(t("error.remarkBreather"));
        } else if (response.status === 429) {
          setAppError(t("error.tooManyRequests"));
        } else {
          setAppError(t("error.cantGenerate"));
        }
        return;
      }
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 503) {
          ...
          setAppError(t("error.cantRefresh"));
        }
        return;
      }
  ...
  }, [selectedRemark, isRegenerating, cooldownRemaining, t]);
    ...
    <main className="relative h-dvh w-full overflow-hidden">
      ...
      <div className="absolute top-safe-area left-4 right-4 z-20 pt-4">
        <SearchBar
          ...
          isLoading={isSearching}
          placeholder={hasRealLocation || viewportState.center ? t("search.placeholder") : t("search.gettingLocation")}
