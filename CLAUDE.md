...
- **Docker**: 3 services (app on host network:3000, postgres:5432, typesense:8108). Dev runs `bun run dev` in container. Production (`make run-public`) uses `docker-compose.prod.yml` overlay to run `bun run build` + `bun run start` in container + Cloudflare Tunnel.
