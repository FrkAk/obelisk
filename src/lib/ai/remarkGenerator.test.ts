...
describe("extractCityFromAddress", () => {
  test("extracts city from standard German address", () => {
    expect(extractCityFromAddress("Marienplatz 1, 80331 Munich, Germany")).toBe(
      "Munich"
    );
  });

  test("extracts city from Turkish address", () => {
    expect(
      extractCityFromAddress("Sultanahmet, Istanbul, T\u00FCrkiye")
    ).toBe("Istanbul");
  });

  test("strips leading postal code from city part", () => {
    expect(
      extractCityFromAddress("Neuhauser Str. 2, 80331 M\u00FCnchen, Germany")
    ).toBe("M\u00FCnchen");
  });
  ...
  test("returns null for single-part address", () => {
    expect(extractCityFromAddress("Single part")).toBeNull();
  });

  test("returns city for two-part address", () => {
    expect(extractCityFromAddress("Street, City")).toBe("Street");
  });
});
